...
def get_dataframe_from_excel():
    ...
    # 最后赋值给 sale_df['小时数']，这样就可以得到一个包含交易小时的新列
...
def product_line_chart(df):
    # 将 df 按'产品类型'列分组，并计算'总价'列的和，然后按总价排序
...
def hour_chart(df):
    ...
    # 使用px.bar函数生成条形图
    # --x=sales_by_hour.index: 条形图的标签是产品类型
    # --y="总价": 条形图的长度表示总价
...
def main_page_demo(df):
    ...
    # 对刚性的结果再次四舍五入，只保留整数，并使用int()函数，表示成整数，增加代码的
    # 可读性
    star_rating_string = "★" * int(round(average_rating, 0))
